...
if $(grep -q 'vagrant' ${ROOTFS}/etc/shadow); then
  ...
  OLDUID=`chroot ${ROOTFS} id -u ubuntu`  # looks up current (old) uid
  ...
  chroot ${ROOTFS} usermod -u $VAGRANTUID vagrant &>> ${LOG}
  chroot ${ROOTFS} groupmod -g $VAGRANTUID vagrant &>> ${LOG}
  chroot ${ROOTFS} find / -uid $OLDUID -exec chown -h $VAGRANTUID {} + &>> ${LOG}
  chroot ${ROOTFS} find / -gid $OLDUID -exec chgrp -h $VAGRANTUID {} + &>> ${LOG}
  ...
  chroot ${ROOTFS} useradd --create-home -s /bin/bash -u $VAGRANTUID vagrant &>> ${LOG}
  echo -n 'vagrant:vagrant' | chroot ${ROOTFS} chpasswd
  ...
  debug 'Creating vagrant user...'
  chroot ${ROOTFS} useradd --create-home -u $VAGRANTUID -s /bin/bash vagrant &>> ${LOG}
